  ...
  return (
    ...
    >
      <span className="flex flex-col px-3 gap-3">
        <strong className="text-style-g-ed text-xl">Experience</strong>

        <p className="text-style-g-e6 text-sm whitespace-nowrap">
          My experiences from published projects.
        </p>
      </span>
      ...
      <div className="flex flex-col gap-2">
        {experiences.map((experience, index) => {
          if (experience.separate) {
            return (
              <div
                key={experience.name_project + index}
                className="flex px-3 mt-6 min-w-full"
              >
                <span className="border-dotted p-3 whitespace-nowrap" />
              </div>
            );
          ...
          if (experience.private) {
            return (
              <Tooltip
                content={experience.toolTip as string}
                key={experience.name_project + index}
              >
                <span className="flex items-center gap-2 p-3 rounded-xl cursor-default">
                  <p className="whitespace-nowrap text-style-g-ed text-sm">
                    {experience.name_project}
                  </p>
